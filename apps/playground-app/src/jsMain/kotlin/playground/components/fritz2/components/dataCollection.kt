package playground.components.fritz2.components
...
import playground.showcase.FakePersons
import playground.showcase.Person
...
fun RenderContext.filterInput(id: String, filterStore: Store<String>) {
    inputField("relative my-4 grow", id) {
        ...
        div("absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none") {
            icon("w-5 h-5 text-primary-600", SolidHeroIcons.magnifying_glass)
        }
        inputTextfield(
...
fun Tag<HTMLTableRowElement>.column(title: String, button: Tag<HTMLDivElement>.() -> Unit) {
    th(
        """sticky top-0 pl-3 py-2.5 z-10
            ...
            | text-left font-medium text-sm text-primary-700""".trimMargin()
    ) {
...
val sortIcons: DataCollection<Person, HTMLDivElement>.DataCollectionSortButton<HTMLButtonElement>.() -> Unit = {
    direction.render {
        icon(
            "text-primary-800 h-4 w-4 mt-1 mr-2", when (it) {
                SortDirection.NONE -> MiniHeroIcons.chevron_up_down
                SortDirection.ASC -> MiniHeroIcons.bars_arrow_up
                SortDirection.DESC -> MiniHeroIcons.bars_arrow_down
            }
        )
    }
}
...
fun RenderContext.dataTableDemo(amount: Int) {
    val persons: List<Person> = FakePersons(amount)
    val storedPersons: Store<List<Person>> = storeOf(persons)
    val selectionStore: Store<List<Person>> = storeOf(persons.take(2))
        ...
        div("flex items-center justify-end h-10 max-w-sm mt-4") {
            ...
            dataCollectionSortButton(
                ...
                direction.render(into = this) {
                    icon(
                        "text-primary-700 h-5 w-5 m-2", when (it) {
                            SortDirection.NONE -> MiniHeroIcons.chevron_up_down
                            SortDirection.ASC -> MiniHeroIcons.bars_arrow_up
                            SortDirection.DESC -> MiniHeroIcons.bars_arrow_down
                        }
                        ...
                        div("w-full flex items-center justify-between p-4 space-x-6") {
                            div("flex-1 truncate") {
                                div("flex items-center space-x-3") {
                                    h3("font-medium text-sm truncate") { +item.fullName }
                                }
                            ...
                            div("-mt-px flex divide-x divide-primary-400") {
                                div("w-0 flex-1 flex") {
                                    a(
                                        ...
                                        icon("w-5 h-5", SolidHeroIcons.envelope)
                                ...
                                div("-ml-px w-0 flex-1 flex") {
                                    a(
                                        ...
                                        icon("w-5 h-5", SolidHeroIcons.phone)
