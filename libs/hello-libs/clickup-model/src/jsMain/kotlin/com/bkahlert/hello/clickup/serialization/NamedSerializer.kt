...
@SerialName("Names")
public data class Named<T>(
    val name: String,
    val value: T,
) {
    public companion object {
        public inline val <T : Any> KClass<T>.derivedName: String
            get() = simpleName?.lowercase() ?: throw IllegalStateException("unable to derive name for $this")

        public fun <T> data(value: T? = null): Named<T?> =
            Named("data", value)

        public inline fun <reified T : Any> ofSingle(single: T): Named<T> =
            Named(T::class.derivedName, single)

        public inline fun <reified T : Any> ofMultiple(multiple: List<T>): Named<List<T>> =
