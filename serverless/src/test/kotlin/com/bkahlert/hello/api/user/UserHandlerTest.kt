...
class UserHandlerTest {
    ...
    @Test
    fun `should validate ID token`() {
        val handler = UserHandler(TestJsonWebTokenValidator())
        val response = handler.handleRequest(event("Bearer ${TestTokens.IdToken}"), TestContext)
        response.body shouldBe "{ \"userId\": \"d2f87456-ed02-49af-8de4-96b9e627d270\" }"
    }

    @Test
    fun `should validate access token`() {
        val handler = UserHandler(TestJsonWebTokenValidator())
        val response = handler.handleRequest(event("Bearer ${TestTokens.AccessToken}"), TestContext)
        response.body shouldBe "{ \"userId\": \"d2f87456-ed02-49af-8de4-96b9e627d270\" }"
    }
